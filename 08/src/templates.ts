 ...
 */
export function handleFunction(functionName: string, nVars: number): string {
  const instruction = [
    `// Declaring function ${functionName} with ${nVars} local variables\n`,
  ];
  // 1. Put label
  instruction.push(`(${functionName})\n`);
  instruction.push("// If local variables are 0, jump to init end");
  instruction.push(`@${nVars}`, "D=A", `@${functionName}_INIT_END`, "D;JEQ\n");
  instruction.push(
    "// If we're here we have at least one local var. We have to push 0 n times."
  );
  instruction.push(`(${functionName}_INIT_START)`);
  instruction.push("@SP", "M=M+1", "A=M-1", "M=0", "D=D-1");
  instruction.push(`@${functionName}_INIT_START`, "D;JNE");
  instruction.push(`(${functionName}_INIT_END)`);
 ...
 *
 * Pushes the return label of the current caller to the stack.
 */
export function handleCallPushRetAddr(
  callerFunc: string,
  timesCalled: number
): string[] {
  const instructions: string[] = [];
  instructions.push(`// Push ${callerFunc} returnAddress`);
  instructions.push(`@${callerFunc}$ret${timesCalled}`);
 ...
 *
 * Saves the value of a given pointer onto the stack
 */
export function handleCallSavePointer(pointer: string): string[] {
  const instructions: string[] = [`// Saving ${pointer}\n`];
 ...
 * handleCallArg
 *
 * Moves the ARG pointer based on the number of arguments
 */
export function handleCallArg(nArgs: number): string[] {
  const instructions: string[] = [`// Moving ARG of SP - 5 - ${nArgs}`];
  instructions.push(`@${nArgs}`);
 ...
 * handleCallFinal
 *
 * Moves LCL pointer to SP position, jumps to callee function,
 * puts caller return label
 */
export function handleCallFinal(functionName: string): string[] {
  const instructions: string[] = [];
  instructions.push("// LCL = SP");
  instructions.push("@SP", "D=M", "@LCL", "M=D");

  instructions.push(`// goto ${functionName}`);
  instructions.push(`@${functionName}`, "0;JMP");
  return instructions;
}
 ...
 *
 * Sets up frame and retAddr variables to perform function return.
 */
export function handleFunctionReturnVars(): string[] {
  const instructions: string[] = [];
 ...
 *
 * Replaces the first argument with the topmost stack value, then brings
 * the stack pointer back to the returned value + 1
 */
export function handleFunctionReturnStackChange(): string[] {
  const instructions: string[] = [];
 ...
 *
 * Restores the pointers to the caller's state, then goes to return address.
 */
export function handleFunctionReturnRestoreCaller(): string[] {
  const instructions: string[] = [];
  instructions.push(...handleFunctionReturnPointerRestore("THAT", 1));
  instructions.push(...handleFunctionReturnPointerRestore("THIS", 2));
  instructions.push(...handleFunctionReturnPointerRestore("ARG", 3));
  instructions.push(...handleFunctionReturnPointerRestore("LCL", 4));
...
/**
 * handleFunctionReturnPointerRestore
 *
 * Restores a stack pointer to the ARG - value state.
 */
function handleFunctionReturnPointerRestore(
  pointer: string,
  value: number
): string[] {
  const instructions: string[] = [`// ${pointer} = *(frame - ${value})`];
  instructions.push(`@${value}`);
  instructions.push("D=A", "@R13", "A=M-D", "D=M", `@${pointer}`, "M=D");
  return instructions;
}
